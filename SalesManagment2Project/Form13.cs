...
namespace SalesManagment2Project
{
    public partial class Form13 : Form
    {
        ...
        if (frm1.durum == false) //form1 deki durum değişkeni false ise 
            {
                double sfiyat;
                sfiyat = double.Parse(textBox1.Text) + double.Parse(textBox1.Text)
               * (double.Parse(textBox2.Text) / 100);
                if (comboBox1.Text.Trim() != "")
                {
                    frm1.bag.Open();
                    frm1.kmt.Connection = frm1.bag;
                    frm1.kmt.CommandText = "INSERT INTO   stokbil(urunAdi, adet, birimFiyat, kdv, satisFiyat) VALUES('" + comboBox1.Text + 
               "','" + textBox3.Text + "','" + textBox1.Text + "','" + textBox2.Text + "','" + sfiyat.ToString() + "') ";
